export class Tablero {
        ...
        while(contadorX < this.datosTablero.ancho){
            ...
            while(contadorY < this.datosTablero.alto){
                ...
                if(datos.casillas[`${contadorX},${contadorY}`]){
                    this.casillas[`${contadorX},${contadorY}`] = new CasillaBoton(rect, "white", `${contadorX},${contadorY}`)
                }else{
                    this.casillas[`${contadorX},${contadorY}`] = new CasillaBoton(rect, "black", `${contadorX},${contadorY}`)
                }
                contadorY++
            }
            contadorX++
    ...
    draw(){
        ...
        if(this.datosTablero != null){
            this.canvas.width = (this.tamañoCasilla+this.margenes) * this.datosTablero.ancho * this.escala
            this.canvas.height = (this.tamañoCasilla+this.margenes) * this.datosTablero.alto * this.escala

            let contadorX = 0
            while(contadorX < this.datosTablero.ancho){
                let contadorY = 0
                while(contadorY < this.datosTablero.alto){
                    if (ctx.isPointInPath(this.casillas[`${contadorX},${contadorY}`].rect, this.posicionMouse[0], this.posicionMouse[1])) {
                        this.coordenadaApuntada = {x:contadorX,y:contadorY}
                        let rect = new Path2D()
                        rect.rect( 
                            (contadorX*( this.tamañoCasilla + this.margenes ) - this.margenes)* this.escala,
                            (contadorY*( this.tamañoCasilla + this.margenes ) - this.margenes)* this.escala,
